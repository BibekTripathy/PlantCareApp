...
void Plants::showDetails() {
  const char *sql =
      "SELECT id, name, species, description, healthStatus FROM plants;";
  sqlite3_stmt *stmt;
...
void Plants::addPlant() {
  ...
  std::cout << "Please enter the following details for the new plant:"
            << std::endl;
  ...
  const char *sql =
      "INSERT INTO plants (name, species, description, healthStatus) "
      "VALUES (?, ?, ?, ?);";
  sqlite3_stmt *stmt;
  ...
  sqlite3_bind_text(stmt, 3, newPlant.description.c_str(), -1,
                    SQLITE_TRANSIENT);
  sqlite3_bind_text(stmt, 4, newPlant.healthStatus.c_str(), -1,
                    SQLITE_TRANSIENT);

  if (sqlite3_step(stmt) != SQLITE_DONE) {
    std::cerr << "Error inserting plant: " << sqlite3_errmsg(db) << std::endl;
  } else {
    std::cout << "Added plant: " << newPlant.name
              << " (ID: " << sqlite3_last_insert_rowid(db) << ")" << std::endl;
  }
...
void Plants::editPlant() {
  ...
  while (true) {
    const char *getSql = "SELECT name, species,description, healthstatus FROM "
                         "plants WHERE id = ?;";
    ...
    std::cout << "\n--- Editing Plant ID: " << plantId << " ---"
              ...
              << "\n5. Exit\n"
              << "Choose property to edit (1-5): " << std::flush;
    ...
    std::string updateSql =
        "UPDATE plants SET " + columnName + " = ? WHERE id =?;";
    sqlite3_stmt *updateStmt;
    if (sqlite3_prepare_v2(db, updateSql.c_str(), -1, &updateStmt, nullptr) !=
        SQLITE_OK) {
      std::cerr << "Failed to prepare statement: " << sqlite3_errmsg(db)
                << std::endl;
      continue;
...
void Plants::searchPlants(const std::string &query) {
  ...
  std::transform(likeQuery.begin(), likeQuery.end(), likeQuery.begin(),
                 ::tolower);
...
void Plants::filterByHealth(const std::string &healthStatus) {
  ...
  if (results.empty()) {
    std::cerr << "Error: No plants found with matching Health-Status: `"
              << healthStatus << "`\n";
  } else {
    showFiltered(results);
  }
...
void Plants::showFiltered(const std::vector<plantData> &filteredPlants) {
  std::cout << "\n=== Filtered Results (" << filteredPlants.size()
            << " plants) ===\n";
